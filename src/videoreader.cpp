...
static void default_vr_allocate(VideoReader::VRImage* image, void*) {
  size_t size = image->stride * image->height;
  image->data = new (std::nothrow) uint8_t[size];
}

static void default_vr_deallocate(VideoReader::VRImage* image, void*) {
...
)
{
  ...
  if (url == "pylon") {
    return std::unique_ptr<VideoReader>(
      new VideoReaderPylon(url, parameter_pairs, extras, allocate_callback, delallocate_callback, log_callback, userdata));
  }
#endif
#ifdef VIDEOREADER_WITH_GALAXY
  if (url.find("galaxy://") == 0) {
    return std::unique_ptr<VideoReader>(
      new VideoReaderGalaxy(url, parameter_pairs, extras, allocate_callback, delallocate_callback, log_callback, userdata));
  }
#endif
#ifdef VIDEOREADER_WITH_FFMPEG
  return std::unique_ptr<VideoReader>(
    new VideoReaderFFmpeg(url, parameter_pairs, extras, allocate_callback, delallocate_callback, log_callback, userdata));
#else
